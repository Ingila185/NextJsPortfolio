  ...
  return (
    <div className="p-10 mx-auto grid grid-cols-1 lg:grid-cols-3 md:grid-cols-2 m-8 gap-6 ">
      {certifications.map((cert, idx) => (
        <div
          key={idx}
          className="max-w-screen  border  rounded overflow-hidden 
        ...
        >
          ...
          <div className="px-6 pt-4 pb-2">
            {cert.courses.map((course, idx) => (
              <span
                key={idx}
                className="inline-block bg-gray-300 hover:bg-gray-800 rounded-full px-3 py-1 text-sm font-semibold text-gray-900 hover:text-gray-400 mr-2 mb-2"
              >
                {course.tag}
              </span>
            ))}
